...
### Часть 1. Базовый TypeScript

1. [Базовые и специальные типы (`any`, `unknown`)](#1-базовые-и-специальные-типы-any-unknown)
2. [Сложные типы: объекты, массивы и Tuple](#2-сложные-типы-объекты-массивы-и-tuple)
3. [Union и Intersection типов (`|` и `&`)](#3-union-и-intersection-типов--и-)
4. [Enum — именованные константы](#4-enum--именованные-константы)
5. [Типизация функций: параметры, `void` и `never`](#5-типизация-функций-параметры-void-и-never)
6. [`type` vs `interface` — что и когда выбирать](#6-type-vs-interface--что-и-когда-выбирать)
7. [Сужение типов (Type Guards через `typeof`, `in`, `instanceof`)](#7-сужение-типов-type-guards-через-typeof-in-instanceof)
8. [`Index Properties`: объекты с динамическими ключами](#8-index-properties-объекты-с-динамическими-ключами)
9. [Дженерики (Generics): гибкие типы для переиспользуемого кода](#9-дженерики-generics-гибкие-типы-для-переиспользуемого-кода)
10. [`Utility Types`: встроенные хелперы (`Partial`, `Pick`, `Omit`, `Record`)](#10-utility-types-встроенные-хелперы-partial-pick-omit-record)

### Часть 2. TypeScript в React

11. [Типизация компонентов, пропсов и `useState`](#11-типизация-компонентов-пропсов-и-usestate)
12. [Типизация событий (`onChange`, `onSubmit`, `onClick`)](#12-типизация-событий-onchange-onsubmit-onclick)
13. [Передача `children`: `JSX.Element` vs `ReactNode`](#13-передача-children-jsxelement-vs-reactnode)

### Часть 3. Практическая миграция (Vite + RTK)

14. [Установка и настройка TypeScript в Vite-проект](#14-установка-и-настройка-typescript-в-vite-проект)
15. [Порядок переименования файлов (`.js` ➔ `.ts` / `.tsx`)](#15-порядок-переименования-файлов-js--ts--tsx)
    - [Как проверить, что проект полностью на TypeScript](#как-проверить-что-проект-полностью-на-typescript)
16. [Redux Toolkit: типизация `slice`, `store` и селекторов](#16-redux-toolkit-типизация-slice-store-и-селекторов)
17. [RTK Query: типизация эндпоинтов и API](#17-rtk-query-типизация-эндпоинтов-и-api)
18. [Редкие кейсы и Legacy: работа с DOM через `as`, темы `styled-components`](#18-редкие-кейсы-и-legacy-работа-с-dom-через-as-темы-styled-components)
...
# 1. Базовые и специальные типы (`any`, `unknown`)
...
# 2. Сложные типы: объекты, массивы и Tuple
...
# 3. Union и Intersection типов (`|` и `&`)
...
# 5. Типизация функций: параметры, `void` и `never`
...
# 6. `type` vs `interface` — что и когда выбирать
...
# 7. Сужение типов (Type Guards через `typeof`, `in`, `instanceof`)
...
# 8. `Index Properties`: объекты с динамическими ключами
...
# 9. Дженерики (Generics): гибкие типы для переиспользуемого кода
...
# 10. `Utility Types`: встроенные хелперы (`Partial`, `Pick`, `Omit`, `Record`)
...
# 11. Типизация компонентов, пропсов и `useState`
...
# 12. Типизация событий (`onChange`, `onSubmit`, `onClick`)
...
# 14. Установка и настройка TypeScript в Vite-проект
...
# 15. Порядок переименования файлов (`.js` ➔ `.ts` / `.tsx`)
...
# 16. Redux Toolkit: типизация `slice`, `store` и селекторов
...
# 17. RTK Query: типизация эндпоинтов и API
...
# 18. Редкие кейсы и Legacy: работа с DOM через `as`, темы `styled-components`
